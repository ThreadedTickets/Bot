"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="8e8e8127-ad37-5775-96f0-9f8202cd615b")}catch(e){}}();
...
function replaceMentions(content, metadata, fallbackUsers) {
    if (!content)
        return "";
    // Replace user mentions: <@123456789> or <@!123456789>
    content = content.replace(/&lt;@!?(\d+)&gt;/g, (_, userId) => {
        const username = metadata.users?.[userId] ||
...
function renderEmbed(embed) {
    ...
    const author = embed.author
        ...
        ${embed.author.icon_url
            ? `<img src="${escape(embed.author.icon_url)}" alt="Author icon" style="width: 20px; height: 20px; border-radius: 50%; margin-right: 8px;">`
            : ""}
        ${embed.author.url
            ? `<a href="${escape(embed.author.url)}" target="_blank" style="font-weight: 500; color: #00a8ff; text-decoration: none;">${escape(embed.author.name)}</a>`
            : `<span style="font-weight: 500;">${escape(embed.author.name)}</span>`}
      </div>`
        : "";
    const title = embed.title
        ? `<div style="font-weight: 600; font-size: 16px; margin-bottom: 6px;">
        ${embed.url
            ? `<a href="${escape(embed.url)}" target="_blank" style="color: #00a8ff; text-decoration: none;">${escape(embed.title)}</a>`
            : escape(embed.title)}
      </div>`
        : "";
    const description = embed.description
        ? `<div style="margin-bottom: 8px; font-size: 14px; line-height: 1.4; white-space: pre-wrap;">${escape(embed.description)}</div>`
        : "";
    const fields = Array.isArray(embed.fields) && embed.fields.length > 0
        ? `<div style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px;">${embed.fields
            .map((f) => {
            const isInline = f.inline ?? false;
            return `<div style="flex: ${isInline ? "1 1 45%" : "1 1 100%"}; min-width: 200px;">
              <div style="font-weight: 500; font-size: 13px;">${escape(f.name)}</div>
              <div style="font-size: 14px; white-space: pre-wrap;">${escape(f.value)}</div>
            </div>`;
        })
            .join("")}</div>`
        : "";
    const image = embed.image?.url
        ? `<div style="margin-top: 10px;"><img src="${escape(embed.image.url)}" alt="Embed image" style="max-width: 100%; border-radius: 4px;"></div>`
        : "";
    const thumbnail = embed.thumbnail?.url
        ? `<div style="margin-left: 16px;">
        <img src="${escape(embed.thumbnail.url)}" alt="Thumbnail" style="width: 80px; height: 80px; object-fit: cover; border-radius: 4px;">
      </div>`
        : "";
    const footerText = embed.footer?.text || "";
    const footerIcon = embed.footer?.icon_url
        ? `<img src="${escape(embed.footer.icon_url)}" alt="Footer icon" style="width: 20px; height: 20px; border-radius: 50%; margin-right: 6px;">`
        : "";
    const timestamp = embed.timestamp
        ? new Date(embed.timestamp).toLocaleString()
        : "";
    const footer = footerText || timestamp
        ? `<div style="display: flex; align-items: center; font-size: 12px; color: #b9bbbe; margin-top: 12px;">
          ${footerIcon}
          <span>${escape(footerText)}${footerText && timestamp ? " • " : ""}${timestamp}</span>
        </div>`
        : "";
...
`;
  ...
  <div>
  <p>${escapeHtml(metadata.name ?? "Unknown")}</p><p>Messages: ${messages.length.toLocaleString()}</p></div>
  ...
  ${Object.entries(users)
        ...
        return `<div style="${colorStyle}">${escapeHtml(meta.username)}${botLabel} — ${count} message${count !== 1 ? "s" : ""}</div>`;
    })
        .join("\n")}</div>
</div>`;
    // Messages panel
    ...
    <div><span class="author" style="color: ${authorColor}">${authorName}</span> <span class="timestamp">${timestamp}</span><span class="timestamp">${msg.edited ? " (Edited)" : ""}</span></div>`;
        // Message Types
        if (msg.type === 4) {
            html += `<div class="system-message"><em>Thread renamed to “${escapeHtml(msg.content)}”</em></div>`;
        }
        else if (msg.type === -1) {
            ...
            const snippet = pinned.content
                ? escapeHtml(pinned.content.length > 50
            ...
            html += `<div class="system-message"><em>${action} <span style="color: ${targetUser?.roleColor ?? "#ddd"}">@${escapeHtml(metadata.users?.[msg.replyTo] || targetUser?.username || "Unknown User")}</span></em></div>`;
        }
        else {
            // Normal message
            if (msg.replyTo && messageMap.has(msg.replyTo)) {
                ...
                const snippet = repliedMsg.content
                    ? escapeHtml(repliedMsg.content.length > 50
...
//# debugId=8e8e8127-ad37-5775-96f0-9f8202cd615b
