"use strict";
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="6d9baabc-b7f0-5df3-ab6c-4646f52022e9")}catch(e){}}();
...
const deleteWebhook_1 = require("../../../utils/bot/deleteWebhook");
const getOrCreateWebhook_1 = require("../../../utils/bot/getOrCreateWebhook");
...
function getLogConfig(settings, group, subcommand) {
    ...
    if (!section)
        return null;
    return section[subcommand] ?? null;
}
const command = {
    type: "slash",
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .setName("channel")
        .addChannelTypes(discord_js_1.ChannelType.AnnouncementThread, discord_js_1.ChannelType.GuildAnnouncement, discord_js_1.ChannelType.GuildText, discord_js_1.ChannelType.PrivateThread, discord_js_1.ChannelType.PublicThread)
        ...
        .addSubcommand((cmd) => cmd
        .setName("create")
        .setDescription("Set a log channel for when a new application is submitted")
        .addChannelOption((option) => option
        .setName("channel")
        .addChannelTypes(discord_js_1.ChannelType.AnnouncementThread, discord_js_1.ChannelType.GuildAnnouncement, discord_js_1.ChannelType.GuildText, discord_js_1.ChannelType.PrivateThread, discord_js_1.ChannelType.PublicThread)
        .setDescription("Choose a channel to assign (leave blank to clear)")
        ...
        .addSubcommand((cmd) => cmd
        .setName("general")
        .setDescription("Set the general log channel where all enabled events without a channel will appear")
        .addChannelOption((option) => option
        .setName("channel")
        .addChannelTypes(discord_js_1.ChannelType.AnnouncementThread, discord_js_1.ChannelType.GuildAnnouncement, discord_js_1.ChannelType.GuildText, discord_js_1.ChannelType.PrivateThread, discord_js_1.ChannelType.PublicThread)
        .setDescription("Choose a channel to assign (leave blank to clear)")
        ...
        .addSubcommand((cmd) => cmd.setName("view").setDescription("View your full logging configuration")),
    async execute(client, data, interaction) {
                ...
                embeds: [
                    {
                        ...
                        title: (0, lang_1.t)(lang, "LOG_COMMAND_VIEW_TITLE"),
                        fields: [
                            // General logging
                            {
                                name: (0, lang_1.t)(lang, "LOG_COMMAND_VIEW_GENERAL_FIELD_NAME"),
                                value: (0, lang_1.t)(lang, "LOG_COMMAND_VIEW_GENERAL_FIELD_VALUE", {
                            ...
                            // Ticket Logging
                            {
                                name: (0, lang_1.t)(lang, "LOG_COMMAND_VIEW_TICKET_FIELD_NAME"),
                                value: (0, lang_1.t)(lang, "LOG_COMMAND_VIEW_TICKET_FIELD_VALUE", {
                            ...
                            // Application Logging
                            {
                                name: (0, lang_1.t)(lang, "LOG_COMMAND_VIEW_APPLICATION_FIELD_NAME"),
                                value: (0, lang_1.t)(lang, "LOG_COMMAND_VIEW_APPLICATION_FIELD_VALUE", {
        ...
        const root = getLogConfig(server.settings.logging, subcommand === "general" ? "general" : subcommandGroup, subcommand);
        if (!root)
            return interaction.reply((await (0, onError_1.onError)(new Error("Can't find log category"))).discordMsg);
        if (!channel && !status)
            return interaction.reply((await (0, onError_1.onError)(new Error("Invalid usage"))).discordMsg);
        if (channel) {
            if (!channel || !channel.isTextBased()) {
                return interaction.reply((await (0, onError_1.onError)(new Error("Invalid channel type"))).discordMsg);
            }
            ...
            if (root.webhook &&
                !isWebhookUsedElsewhere(subcommand === "general"
                    ? "general"
                    : `${subcommandGroup}.${subcommand}`, root.webhook, server.settings.logging)) {
                await (0, deleteWebhook_1.deleteWebhookByUrl)(root.webhook);
            }
            root.webhook = await (0, getOrCreateWebhook_1.getOrCreateWebhook)(webhookChannel, client.user);
            interaction.reply({
                flags: [discord_js_1.MessageFlags.Ephemeral],
                content: `${(0, lang_1.t)(lang, `LOG_CHANNEL_SET${root.enabled ? "_AND_ENABLED" : "_AND_DISABLED"}`)} ${root.enabled && root.webhook
                    ? (0, lang_1.t)(lang, "TEST_NOTIFICATION_ALSO_SENT")
                    : ""}`,
            });
            if (root.webhook)
                (0, sendLogToWebhook_1.postLogToWebhook)(client, root, {
                    embeds: [
                        {
                            color: parseInt(colours_1.default.success, 16),
                            description: (0, lang_1.t)(lang, "TEST_NOTIFICATION"),
                        },
                    ],
                });
        }
            ...
            interaction.reply({
                flags: [discord_js_1.MessageFlags.Ephemeral],
                content: `${(0, lang_1.t)(lang, `LOG_CHANNEL_UNSET`)}`,
            });
        }
        else if (status) {
            ...
            interaction.reply({
                flags: [discord_js_1.MessageFlags.Ephemeral],
                content: `${(0, lang_1.t)(lang, `LOG_CHANNEL_${root.enabled ? "ENABLED" : "DISABLED"}`)}`,
            });
        }
        await server.save();
        await (0, updateServerCache_1.updateServerCache)(interaction.guildId, server);
    },
};
exports.default = command;
function isWebhookUsedElsewhere(currentKey, webhookUrl, loggingConfig) {
    for (const [typeKey, logTypes] of Object.entries(loggingConfig)) {
        if (typeof logTypes !== "object")
            continue;
        for (const [logKey, config] of Object.entries(logTypes)) {
...
//# debugId=6d9baabc-b7f0-5df3-ab6c-4646f52022e9
